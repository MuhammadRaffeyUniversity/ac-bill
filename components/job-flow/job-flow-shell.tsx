...
export function JobFlowShell({ rows, selectedId, search, showWorkspace, children }: { rows: JobFlowQueueRow[]; selectedId?: string; search: string; showWorkspace: boolean; children: React.ReactNode }) {
      ...
      <section className={showWorkspace ? "min-w-0" : "hidden lg:block"}>
        <div className="mx-auto max-w-5xl p-4 md:p-6">
          <div className="mb-4 flex items-center justify-between gap-3">
            <Link href="/jobs" className={cn(buttonVariants({ variant: "ghost", size: "sm" }), "lg:hidden")}><ArrowLeftIcon data-icon="inline-start" />Back to jobs</Link>
