...
import { createInvoice, recordPayments, type BillingActionState } from "@/src/lib/billing/actions";
...
const initialBillingActionState: BillingActionState = {};

export function InvoiceWorkspace({ completedJobs, invoices, initialJobId }: { completedJobs: CompletedJob[]; invoices: Invoice[]; initialJobId?: string }) {
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]"><InvoiceCreator jobs={completedJobs} initialJobId={initialJobId} /><InvoiceList invoices={invoices} /></div>;
}

function InvoiceCreator({ jobs, initialJobId }: { jobs: CompletedJob[]; initialJobId?: string }) {
