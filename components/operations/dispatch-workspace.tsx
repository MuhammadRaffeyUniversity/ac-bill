...
export function DispatchWorkspace({ jobs, teams, filters }: { jobs: Job[]; teams: Team[]; filters: Filters }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
    ...
    <Card className="h-fit"><CardHeader><CardTitle>Assign a team</CardTitle><CardDescription>Suggestions use service-area tags first, then today&apos;s open workload. You can always override.</CardDescription></CardHeader><CardContent>
      {teams.length && jobs.length ? <form action={formAction} className="grid gap-4"><Field label="Job"><select name="jobId" required className={controlClass}><option value="">Choose job</option>{jobs.map((job) => <option key={job.id} value={job.id}>{job.customer} - {job.serviceType.toLowerCase()}</option>)}</select></Field><Field label="Active team"><select name="teamId" required className={controlClass}><option value="">Choose team</option>{teams.map((team) => <option key={team.id} value={team.id}>{team.name}{team.region ? ` - ${team.region}` : ""} ({team.activeJobs} open today)</option>)}</select></Field><Field label="Assignment note"><textarea name="note" maxLength={500} className={`${controlClass} min-h-20 resize-y`} placeholder="Optional handoff context or override reason" /></Field><FormNotice state={state} /><Button type="submit" disabled={isPending}><ArrowRightLeftIcon data-icon="inline-start" />{isPending ? "Assigning..." : "Assign job"}</Button></form> : <Empty text={teams.length ? "Create a job before assigning it." : "No active teams are configured yet."} />}
    </CardContent></Card>
  </div>;
}
