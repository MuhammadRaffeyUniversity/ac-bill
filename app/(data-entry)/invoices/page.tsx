...
export default async function InvoicesPage() {
  ...
  return <main className="min-h-screen bg-muted/30 px-4 py-6 md:px-8 md:py-10"><div className="mx-auto grid max-w-7xl gap-6"><header><h1 className="text-2xl font-semibold">Invoices and payments</h1><p className="mt-1 text-sm text-muted-foreground">Issue invoices after service completion, then record every payment method separately.</p></header><InvoiceWorkspace completedJobs={completedJobs.map((job) => ({ id: job.id, customer: job.customer.name, serviceType: job.serviceType, requestedAt: job.requestedAt?.toISOString() ?? null }))} invoices={invoices.map((invoice) => ({ id: invoice.id, invoiceNumber: invoice.invoiceNumber, status: invoice.status, total: Number(invoice.total), paid: invoice.payments.reduce((sum, payment) => sum + Number(payment.amount), 0), customer: invoice.job.customer.name, serviceType: invoice.job.serviceType, issuedAt: invoice.issuedAt?.toISOString() ?? null }))} /></div></main>;
}
